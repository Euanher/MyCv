...
DELIMITER $$
USE `mydb`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `Register_Log`()
BEGIN
 SELECT 
    Register_Username, 
    Register_Allocate, 
...
-- View `Register_Log`
-- -----------------------------------------------------
DROP VIEW IF EXISTS `Register_Log` ;
SHOW WARNINGS;
USE `mydb`;
CREATE OR REPLACE VIEW Register_Log AS
SELECT User_name, 
User_email, 
User_password, 
User_QUESTION
